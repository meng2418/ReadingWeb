package com.weread.service.impl.bookshelf;
...
public class BookshelfServiceImpl implements BookshelfService {
    ...
    public MarkFinishedVO markBookFinished(Integer bookId, Integer userId) {
        ...
        // 创建BookData用于里程碑检查
        com.weread.service.impl.user.ReadingServiceImpl.BookData bookData = new com.weread.service.impl.user.ReadingServiceImpl.BookData();
    ...
    public List<?> getUserBooksByStatus(String status, Integer userId, boolean isSimple) {
        ...
        if (status == null || status.isEmpty()) {
            shelfList = bookshelfRepository.findByUserId(userId);
        } else {
            shelfList = bookshelfRepository.findByUserIdAndStatus(userId, status);
        }

        // 2. 返回完整VO or 简化VO
        if (isSimple) {
            // 适配 /bookshelf/{status}：只返回cover/title
            return shelfList.stream().map(shelf -> {
                SimpleBookVO vo = new SimpleBookVO();
                BeanUtils.copyProperties(shelf.getBook(), vo);
                return vo;
            }).collect(Collectors.toList());
        } else {
            // 适配 /bookshelf：返回完整信息+readingStatus
            return shelfList.stream().map(shelf -> {
                BookShelfVO vo = new BookShelfVO();
                BeanUtils.copyProperties(shelf.getBook(), vo);
                vo.setStatus(shelf.getStatus());
    ...
    public String batchRemoveBooksFromShelf(BatchRemoveDTO dto, Integer userId) {
        ...
        // 调用Repository批量删除
        bookshelfRepository.deleteByUserIdAndBookIdIn(userId, bookIds);
        return "批量移除书籍成功";
    }
}
