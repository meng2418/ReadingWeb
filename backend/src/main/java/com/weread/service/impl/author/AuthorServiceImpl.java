...
public class AuthorServiceImpl implements AuthorService {
    ...
    public AuthorDetailVO getAuthorDetail(Integer authorId, Integer currentUserId) {
        ...
        Long authorIdLong = authorId.longValue();

        // 2. 查询作者的所有作品
        List<BookEntity> books = bookRepository.findByAuthorIdAndIsPublishedTrue(authorIdLong);

        // 3. 统计作品数量
        int workCount = bookRepository.countByAuthorIdAndIsPublishedTrue(authorIdLong);
